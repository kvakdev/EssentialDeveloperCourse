...
extension FeedStoreSpecs where Self: XCTestCase {
    func test_retrieve_hasNoSideEffectsOnEmptyCache(on sut: FeedStore) {
        expect(sut, toRetrieve: .empty)
        expect(sut, toRetrieve: .empty)
    }
    
    func test_retrieve_deliversTheInsertedValues(on sut: FeedStore) {
    ...
    func test_retrieve_deliversTheSameValues(on sut: FeedStore) {
        let feed = uniqueImageFeed().local
        let timestamp = Date()
        let expectedResult = FeedRetrieveResult.found(feed: feed, timestamp: timestamp)
    ...
    func test_retrieve_deliversErrorOnCurruptedData(on sut: FeedStore) {
        insertCurruptedData(url: testStoreUrl)
        
        expect(sut, toRetrieve: .failure(anyNSError()))
    }
    
    func test_insert_overwritesLatestResults(on sut: FeedStore) {
    ...
    func test_delete_removesCacheOnNonEmptyCache(on sut: FeedStore) {
        let feed = uniqueImageFeed().local
        let timestamp = Date()
    ...
    func test_operations_completeSerially(on sut: FeedStore) {
